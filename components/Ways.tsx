  ...
  return (
    <section id="buy" className="section-y bg-bone-2">
      <div className="shell">
        ...
        <div className="grid border-t border-ink/12 md:grid-cols-3">
          {ways.map((w, i) => (
            <Reveal
              key={w.title}
              delay={i * 80}
              className={`flex flex-col gap-3 border-b border-ink/12 py-8 md:border-b-0 md:border-r md:px-8 md:pb-10 md:pt-9 ${i === 0 ? "md:pl-0" : ""} ${i === ways.length - 1 ? "md:border-r-0 md:pr-0" : ""}`}
            >
              <h3 className="font-display text-[1.5rem] leading-tight">
                {w.title}
              </h3>
              <p className="text-[0.9375rem] text-muted">{w.note}</p>
              <a
        ...
        <p className="mt-8 max-w-[62ch] text-[0.8125rem] text-muted">
